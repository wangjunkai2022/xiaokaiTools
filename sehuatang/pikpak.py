...
async def officeDownload(magnet, path):
    try:
        logging.info("下载磁力连接{}\n到pikpak\n{}".format(magnet, path))
        if not client.access_token or not client.refresh_token or not client.user_id:
            await asyncio.wait_for(client.login(), timeout=50000)
        # 获取pikpak的path的id
        ...
        while True:
            ...
            logging.info("{} 的 下载状态：{}".format(file_name, result))
        ...
        logging.info("pikpak 保存完成：{} 保存路径：{}".format(file_name, path))
        return file_name
    except:
        logging.error("pikpak 保存失败：{} 暂时保存路径：{}".format(magnet, path))
        return ""
