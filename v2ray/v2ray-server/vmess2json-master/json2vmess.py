...
def parse_inbounds(jsonobj):
    ...
    for v in vmesses:
        ...
        if option.debug:
            msg = lambda \
                x: "{ps} / {net} / {add}:{port} / net:{net}/aid:{aid}/host:{host}/path:{path}/tls:{tls}/type:{type}".format(
                **x)
            print(msg(v))
        print()
        print()
        print(link)
        print()
        qr_go = qr.Qrcode(invert_flag=True,
    ...
    if "settings" in inbound and "clients" in inbound["settings"]:
        for c in inbound["settings"]["clients"]:
            vobj = dict(
                ...
                v="2", tls=_tls, add=_add, port=_port, type=_type, net=_net, path=_path, host=_host,
                ps="{}/{}".format(_add, _net))
