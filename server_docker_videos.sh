...
sudo docker run -d --name=sonarr -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -p 8989:8989 -v ~/videos/tools/sonarr:/config -v ~/videos/data/电视:/tv --restart always linuxserver/sonarr

mkdir -p ~/videos/tools/radarr
sudo docker run -d --name=radarr -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -p 7878:7878 -v ~/videos/tools/radarr:/config -v ~/videos/data/电影:/movies --restart always linuxserver/radarr:latest
...
sudo docker run -d --name -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC embyserver -v ~/videos/tools/embyserver:/config -v ~/videos/data/电视:/mnt/电视 -v ~/videos/data/电影:/mnt/电影 -p 8096:8096 -p 8920:8920 --env UID=1000 --env GID=1000 --restart always emby/embyserver:latest
